...
A secure, auto-rotating password generator written in Python. It continuously generates strong random passwords at randomized intervals (5–15 seconds), making it useful for demos, testing, or quickly producing multiple secure passwords.
...
-  **Shuffled output** — character positions are randomized using a Fisher–Yates shuffle driven by `secrets.randbelow`
-  **Cross-platform** — clears the console on both Windows (`cls`) and Unix-like systems (`clear`)
-  **Press Enter to stop** — gracefully stops and displays the last generated password
...
- Python **3.6+** (the `secrets` module is available since Python 3.6)
...
1. **Initialization** — one character is picked from each required category.
2. **Filling** — the remaining characters are randomly chosen from the full alphabet.
3. **Shuffle** — the list is shuffled using a secure Fisher–Yates algorithm.
4. **Loop** — the password is displayed, then the script sleeps for a random delay before generating the next one.
5. **Stopping** — a daemon thread waits for `Enter` and signals the main loop via a `threading.Event`.
